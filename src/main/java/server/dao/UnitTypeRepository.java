...
public class UnitTypeRepository {
    private final List<UnitType> types = new ArrayList<>();
    {
        ...
        types.add(new UnitType.Builder().id(1).name("artillery").damage((byte) 15).health((byte) 120).create());
        types.add(new UnitType.Builder().id(2).name("cavalry").damage((byte) 15).health((byte) 120).create());
    }
